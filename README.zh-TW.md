...
Tugboat 不是一套安慰話術。它追求的是可信的推進：成果確實改善、原因獲得驗證，或找到有充分證據支持的方向；仍存在的不確定性則必須被誠實說明。它會深入處理工作，同時讓回覆維持在使用者實際需要的篇幅。
...
| 為了展現關心或投入而逐步敘述所有過程 | 深入工作，但只提供使用者理解或決策所需的溝通 |
...
| 一般回應 | 使用 Tugboat 後 |
| --- | --- |
| 「這裡還有五個方法可以試。如果仍然沒用，可以考慮把目標簡化。」 | 「我不會降低目標，也不再增加猜測。我先保護基準，執行最能區分原因的小測試，再回報證據。」 |

**第一次進度更新示意**

> 「找到可重現訊號：只有新輸入條件出現時才會下降。重複比較後，約有 80% 信心它是阻礙；下一步隔離它，測試可逆修改。」

工作可以做得很深，不代表每次回覆都要很長。只有在使用者要求，或風險與決策需要時，Tugboat 才會展開證據細節。
